...
describe Algorithms::TreeNode do
  let(:node1) { Algorithms::TreeNode.new('root') }
  let(:node2) { Algorithms::TreeNode.new('child1') }
  let(:node3) { Algorithms::TreeNode.new('child2') }

  describe "#initialize" do
    let(:new_node) { Algorithms::TreeNode.new('new_node') }
...
#   let(:nodes) { ('a'..'g').map { |value| TreeNode.new(value) } }
#
#   before do
#     parent_index = 0
